...
public class PuzzleLevel2Manager : MonoBehaviour, IPuzzleLevelManager
{
    ...
    public float templateRotationOffsetZ = 90f;
    ...
    public float pieceRadius = 0.18f;
    ...
    IEnumerator AutoScanRoutine()
    {
        ...
        while (currentState == GameState.Scanning)
        {
            ...
            if (scanAttempts >= 8)
            {
                ...
                // --- GLOBÁLNÍ LOG (Obtížnost 2) ---
                SessionManager.Instance?.SendSystemStatus("No_Table_Found", "MrPuzzle", 2, totalIterations);
    ...
    void GridScanForTable()
    {
        ...
        if (validTablePoints.Count > 10)
        {
            ...
            // --- GLOBÁLNÍ LOG (Obtížnost 2) ---
            SessionManager.Instance?.SendSystemStatus("active", "MrPuzzle", 2, totalIterations);
    ...
    void PlayIteration(int index)
    {
        ...
        // OPRAVA SIROTKÙ: Pøidáno slovo 'transform'
        ...
        {
            piece.SetupPiece(this); // PROPOJENO PØES INTERFACE

            // Pokusíme se najít bezpeèné místo 
            ...
            if (safePosOrNull == null)
            {
                ...
                // --- GLOBÁLNÍ LOG (Obtížnost 2) ---
                SessionManager.Instance?.SendSystemStatus("error_no_space", "MrPuzzle", 2, totalIterations);

                currentState = GameState.Evaluating;
                return;
            }

            // Pokud je to v pohodì, rozbalíme hodnotu z nullablu a pokraèujeme normálnì
