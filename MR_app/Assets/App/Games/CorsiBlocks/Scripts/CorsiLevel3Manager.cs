...
public class CorsiLevel3Manager : MonoBehaviour, ICorsiLevelManager
{
    ...
    // Herní logika
    private List<CorsiBlock> spawnedBlocks = new List<CorsiBlock>();
    ...
    private float levelStartTime; // TELEMETRIE: Stopky úrovnì
    ...
    {
        CreateStatusText("Level 3: Skenuji stùl...");
        int scanAttempts = 0;
        ...
        while (currentState == GameState.Scanning)
        {
            ...
            if (scanAttempts >= 8)
            {
                ...
                // --- GLOBÁLNÍ LOG SELHÁNÍ SKENERU ---
                int totalIt = SessionManager.Instance != null ? SessionManager.Instance.TotalIterations : 0;
                SessionManager.Instance?.SendSystemStatus("No_Table_Found", "CorsiBlocks", 3, totalIt);
    ...
    void GridScanForTable()
    {
        ...
        if (validTablePoints.Count > 10)
        {
            ...
            if (SpawnCorsiBoard(exactTableCenter, tableRotation))
            {
                ...
                UpdateStatusText("Stùl nalezen! Nejtìžší zkouška zaèíná...");

                // --- GLOBÁLNÍ LOG ÚSPÌCHU ---
                SessionManager.Instance?.SendSystemStatus("active", "CorsiBlocks", 3, maxIterations);
            ...
            else
            {
                ...
                // --- GLOBÁLNÍ LOG CHYBY ---
                SessionManager.Instance?.SendSystemStatus("error_no_space", "CorsiBlocks", 3, maxIterations);
    ...
    // --- INTEGRACE SPACE OPTIMIZERU & ÚKLID DÌTÍ ---
    bool SpawnCorsiBoard(Vector3 center, Quaternion rotation)
    {
        ...
        int attempts = 0;
        int maxAttempts = 500; // Zvýšeno na 500 pokusù pro vyšší spolehlivost
        ...
        while (spawnedBlocks.Count < totalBlocks && attempts < maxAttempts)
        {
            ...
            // tady posíláme testovací bublinu 15 cm do vzduchu nad desku stolu
            ...
            if (Physics.Raycast(rayOrigin, Vector3.down, out RaycastHit hit, 0.4f, layerMask))
            {
                ...
                if (Mathf.Abs(hit.point.y - (center.y - spawnHeightOffset)) < 0.15f)
                {
                    ...
                    CorsiBlock blockScript = newObj.GetComponent<CorsiBlock>();
                    if (blockScript != null)
                    {
    ...
    IEnumerator PlayIterationRoutine()
    {
        ...
        UpdateStatusText($"Dívejte se! ({currentIteration}/{maxIterations})");
        ...
        UpdateStatusText("<color=orange>Nyní zopakujte poøadí POZPÁTKU!</color>");
    ...
    public void PlayerSelectedBlock(int blockID)
    {
        ...
        // Vypoèítáme, jaký index ze zadu právì oèekáváme
        int expectedIndex = targetSequence.Count - 1 - playerInputIndex;

        if (blockID == targetSequence[expectedIndex])
        {
            // TELEMETRIE: Úspìšný krok pozpátku
            LogEvent("Corsi_Touch_Success", $"Block_{blockID}");
        ...
        {
            // TELEMETRIE: Chyba v pamìti pozpátku
            LogEvent("Corsi_Touch_Miss", $"Block_{blockID}");
    ...
    IEnumerator HandleRoundEnd(bool isCorrect)
    {
        if (isCorrect)
        {
            ...
            // TELEMETRIE: Nahlášení splnìného kola
            ...
            {
                UpdateStatusText("<color=green>Kompletní Corsi test dokonèen!</color>");
    ...
    // ÚKLID CANVASU PØI PØEPNUTÍ HRY
    void OnDestroy()
    {
        if (spawnedCanvasObj != null) Destroy(spawnedCanvasObj);
    }
