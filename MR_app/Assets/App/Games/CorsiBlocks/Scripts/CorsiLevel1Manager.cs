...
public class CorsiLevel1Manager : MonoBehaviour, ICorsiLevelManager
{
    ...
    private float levelStartTime; // MĚŘENÍ REAKČNÍHO ČASU
    ...
    public void StartLevel(int totalIterations)
    {
        ...
        currentIteration = 1;
        isGameRunning = true;
        levelStartTime = Time.time; // Odstartování stopek
    ...
    IEnumerator AutoScanRoutine()
    {
        ...
        while (currentState == GameState.Scanning)
        {
            ...
            if (scanAttempts >= 8)
            {
                ...
                SessionManager.Instance?.SendSystemStatus("No_Table_Found", "RotationCube", 1, totalIt);
    ...
    void GridScanForTable()
    {
        ...
        if (validTablePoints.Count > 10)
        {
            ...
            if (SpawnCorsiBoard(exactTableCenter, tableRotation))
            {
                ...
                UpdateStatusText("Stůl nalezen! Připravte se...");

                // --- GLOBÁLNÍ LOG ÚSPĚCHU ---
                SessionManager.Instance?.SendSystemStatus("active", "CorsiBlocks", 1, maxIterations);
            ...
            else
            {
                ...
                // --- GLOBÁLNÍ LOG CHYBY ---
                SessionManager.Instance?.SendSystemStatus("error_no_space", "CorsiBlocks", 1, maxIterations);
    ...
    bool SpawnCorsiBoard(Vector3 center, Quaternion rotation)
    {
        ...
        // Zvýšili jsme počet pokusů, aby měl algoritmus větší šanci poskládat to i na menším stole
        int attempts = 0;
        int maxAttempts = 500;
        ...
        while (spawnedBlocks.Count < totalBlocks && attempts < maxAttempts)
        {
            ...
            // 1. Lokální kontrola vzdálenosti mezi kostkami
            bool tooClose = false;
    ...
    IEnumerator PlayIterationRoutine()
    {
        ...
        UpdateStatusText($"Dívejte se! (Kolo {currentIteration}/{maxIterations})");
        ...
        UpdateStatusText("Nyní zopakujte pořadí!");
    ...
    IEnumerator HandleRoundEnd(bool isCorrect)
    {
        if (isCorrect)
        {
            ...
            {
                UpdateStatusText("<color=green>Úroveň dokončena!</color>");
